...
import { Theater, Calendar, Search, Check, Plus, Trash, User, Activity } from 'lucide-react';

export default function HistoriqueSalles() {
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            const response = await adminApi.getFilteredSalleHistory(params);
    ...
    const fetchStats = async () => {
        try {
            const response = await adminApi.getGlobalSalleHistoryStats();
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex items-center justify-start gap-2">
                    <Theater className="w-4 h-4 text-red-500" />
    ...
    return (
        <HistoriqueTable
            title="Historique Salles"
            icon={Theater}
            data={data}
            columns={columns}
            isLoading={isLoading}
            stats={stats}
            pageName="HistoriqueSalles"
            exportEndpoint="/admin/historique/salles/export/excel"
            exportParams={{
            ...
            filters={
                <>
                    ...
                    <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2">
                        ...
                        <input
                            type="text"
                            placeholder="Rechercher (Admin ou Salle)..."

...
export default function AfficherCommerciauxDesactives() {
    ...
    const fetchUsers = async () => {
        setLoading(true);
        try {
            const response = await adminApi.getUsers("COMMERCIAL", false);
    ...
    const handleToggleStatus = async (user) => {
        if (window.confirm(`Voulez-vous réactiver ce commercial ?`)) {
            try {
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-white">Archives Commerciaux</h2>
                        <p className="text-zinc-500 text-sm">Liste des commerciaux désactivés</p>
                    </div>
                </div>
                <button
